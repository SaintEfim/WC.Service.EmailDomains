...
using WC.Service.EmailDomains.Domain.Services.EmailDomain.Validators.Create;

namespace WC.Service.EmailDomains.Domain.Tests.Services.EmailDomain.Validators;

public class EmailDomainCreateDbValidatorTests
{
    private static EmailDomainCreateDbValidator GetValidator(
        IMock<IEmailDomainRepository> repository)
    {
        ...
        builder.RegisterType<EmailDomainCreateDbValidator>();

        builder.Populate([]);

        var container = builder.Build();
        return container.Resolve<EmailDomainCreateDbValidator>();
    }

    [Fact]
    public async Task EmailDomain_Positive_Create_New_Record()
    {
    ...
    [Fact]
    public async Task EmailDomain_Negative_Create_New_Record_Has_Duplicate()
    {
        var emailDomain = EmailDomainData.EmailDomainModel();
